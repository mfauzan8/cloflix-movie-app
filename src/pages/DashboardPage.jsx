              ...
              <div key={item.id} className="relative">
                ...
                <div className="absolute top-1/4 left-10 z-2 text-white p-2 rounded w-1/2 text-start">
                  <div className="flex flex-col gap-3">
                    <p className="text-5xl font-extrabold uppercase drop-shadow-lg">{item.original_title || item.original_name}</p>
                    ...
                    <p className="text-lg mt-10 text-justify drop-shadow-lg">{item.overview}</p>

                    <div className="flex gap-3 items-center text-2xl">
                      <FaStar className="text-yellow-400" />
                      <div className="font-bold border-e-2 pe-3 drop-shadow-lg">{item.vote_average.toFixed(1)} </div>
                      <BiSolidLike className=" text-xl drop-shadow-lg" />
                      <span className="font-normal text-xl drop-shadow-lg"> {item.vote_count}</span>
                    </div>
                    <div className="flex w-2/5 gap-2">
                      {/* <button className="button-movie bg-red-800">Details</button> */}
                      <button className="flex items-center gap-1 button-movie bg-black " onClick={() => navigate(`/${item.media_type}/detail/${item.id}`)}>Detail</button>
                    </div>
                  </div>
                </div>
              </div>
